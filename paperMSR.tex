...
This paper tries to shed some light on this topic, by analyzing the relationship of bug fixes with their previous commits. To this end, we conducted an observational study on bug reports, their fixes, and their corresponding previous commits for the OpenStack project. Our results show that the assumption that bugs have been introduced in the previous commit does not hold for a large fraction of the bugs analyzed. \grex{maybe we could say something more here}. \gema{podriamos escribir algunos porcentajes de lo que hemos encontrado, algo asi como que un 37\% de los commits previos no son responsables de introducir niingun bug}
\end{abstract}
...
\grex{This last paragraph should be clarified, and more on papers using the SZZ algorithm! Maybe we should state how many papers in total exist based or using SZZ in Google Scholar.}
\gema{ He buscado en google schoolar las entradas que tiene SZZ algoritm y aparecen 1170 resultados desde 2006 que es cuando sacaron el articulo, pero alguna de las entradas solo contine algorithm y no SZZ algoritm}
...
\gema{Hay que incluir lo que nuestro articulo aporta al estado actual} \grex{Buena idea, pero eso es mejor ponerlo en la intro.}\gema{Ya esta puesto}
...
\begin{itemize}
  ...
  \item The changes to the source code, as sometimes neither the descriptions nor the comments by developers and reviewers in the Launchpad and Gerrit of each ticket, clarified the underlying ticket. \grex{what comments?}\gema{los comentarios de los reviewers que podemos ver en Luanchpad y Gerrit }
\end{itemize}
...
We classify a total of 459 tickets using the tool, resulting in 918 reviews. Only those tickets classified as bug reports by both researchers were considered in the next stage, which analyzes if the cause of the bug was introduced in their previous commits.
...
R3 & (188) 56\% & (116) 35\% & (30) 9\% & 334 \\ \hline
Agree & (209) 72\% & (74) 25\% & (9) 3\% & 292 \\
\bottomrule[0.3mm]
\end{tabular} %}
\caption{Statistics for each researcher as a result of the classification process. For each researcher R, the number of tickets (and percentages) classified into the three groups is given. The \emph{Agree} row gives the number of tickets (and percentages) were two researchers agreed.}
\label{tab:2}
%\end{center}
\end{table}
...
In this stage we analyzed the 189 tickets classified as bug reports and their previous commits.
The possible outcome was one of the following three options:
...
This analysis takes into account that the bug could have been inserted in many lines belong to many different previous commits, but in fact, not all of them could cause the bug. Sometimes happens that in the previous commit some lines had been copied from further previous commits, or comments may have been modified, or blank spaces/lines have been added. Thus, the cause could be found in a single previous commit, in many or even in none. \grex{repasar este parrafo} \gema{Que tal ahora? esta mas claro?}

We identified a total of 348 previous commits which could be the cause of the 189 bug reports under analysis, we analyzed the bug reports together with their previous commits discarting these cases where the previous commit was false positives such as blank lines, changes in comments or even a change in the version of the file., so in total we had analyzed 308 previous commit.


As can be seen in Table~\ref{tab:responsability}, from the 308 previous commit, 152 of them have been considered the cause of the bug, whereas 114 were not the cause. We were unable to identify the cause of the bug in 42 cases, due to there was only added lines or our knowleadge about the code. 

As can be seen in Table~\ref{tab:responsability}, in 152 cases the previous commits have been considered the cause of the bug, whereas 114 times they were not the cause. We were unable to identify the cause of the bug in 42 cases, and discarded 40 more because they ere false positives such as blank lines, changes in comments or even a change in the version of the file. \grex{explicar que es eso de noise!} \gema{Con Noise me referia a los falsos positivos que puede haber en los commits previos, las lineas copiadas de otros commits o lineas en blanco, comentarios ...}
...
If we attend to how many previous commits each of the 189 bug reports analyzed had, we see that 131 only had a previous commit, whereas 58 had more than one previous commit. As can be seen from Table~\ref{tab:secondStage}, from the 131 unique previous commits, 65 were the cause of the bug, and 30 not caused the failure. For the 58 bugs that had more than one previous commit, a total number of 179 previous commits were identified; of them 86 were the cause of the bug, while 82 were not.
...
\gema{- Ademas he mirado como o porse distribuyen los commits en cada bug report dentro de cada proyecto analizado, y he visto que por ejemplo Neutron presentaba 11 bugs report con solo un ticket previo, 3 bugs report con dos identificadores diferentes para los commit previos, 2 bug reports que presentaban  3 commit previos... etc.}\\
\gema{- Finalmente realice ora clasificacion en la que tenia en cuenta el numero de commits que fueron causantes del error para un mismo bug report. Al analizar los bug reports encontre que en aquellos que presenstaban mas de un commit previo no todos los commit eran causantes del error, y en ocasiones solamente habia un commit que causo el error y los otros no o por el contrario habia bugs report en los que ninguno de sus commits previos era responsable y otras ocasiones en las que todos los commits previos eran responsables. Por tanto la tabla 7 muestra cual es la responsabilidad (tods responsables, al menos uno ningun responsable y desconozco la responsabilidad) de los coommits previos presentes en un bug report. Por ejemplo, bugs reports que tenian dos commits previos, en 4 de ellos los dos commits previous eran responsables, mientras que en 9 bug report habia al menos un responsable y en otros 4 bug report ninguno de los dos commit previos eran responsables.}  
We also studied the distribution of the number of previous commits for each Bug Report. As shown in Table~\ref{tab:secondStage2}, the number of commits that can be considered as previous is 1, followed by 2 commits.
...
Finally, we were interested in analyzing, for those cases where more than one previous commit exist, how many of them inserted the bug in the code source. Results are given in Table~\ref{tab:secondStage3}; in 8 bug reports all the previous commits were identified as the cause, in 30 bug reports at least one of the previous commits caused the bug, and in 11 bug reports none of the previous commits introduced the bug.
...
\gema{Deberia hablar sobre los siguientes puntos:} \grex{Puedes elaborar un poco mas cada punto, aunque sea en castellano?}
\begin{enumerate}
...
\end{enumerate}
\gema{anadir ademas que los resultados optenidos presentan una oposicion a la idea actual de que el previo commit introdujo el error}
...
\gema{Incluir un link con la web en la que pondre todos los datos y scripts utilizados para los datos.}
