...
As can be seen in Table~\ref{tab:responsability}, from the 308 previous commits, 152 have been considered to be the cause of the bug, whereas 114 were identified as not being the cause. We were unable to decide in 42 cases or due to our limited knowledge about the code. Figure~\ref{fig:added} shows an example of fix bug commit where only lines have been added. In such a case, we are not able to know which previous commit is the one that caused the error, so we classify it as \textit{Undecided}. %\grex{Gema, puedes poner un ejemplo de estas dos cosas? Uno de solo added lines y otro de limited knowledge. Si el ejemplo es verdadero, mejor.}
\gema{Me falters el ejemplo de limited knowledge, cuando encuentre uno bueno lo anado}
In addition, we discarded 40 more previous commits because they were false positives (\emph{noise}) such as blank lines, changes in comments or even a change in the version of the file.

\begin{figure}[ht]
\centering
\includegraphics[height=2.5cm]{addedlines.png}
...
If we attend to how many previous commits each of the 189 bug reports analyzed had, we see that 131 only had a previous commit as the example \ref{fig:3}, whereas 58 had more than one previous commit as the one in ~\ref{fig:manyprevious}. In Table~\ref{tab:secondStage}, from the 131 unique previous commits, 65 were the cause of the bug, and 30 not caused the failure. For the 58 bugs that had more than one previous commit, a total number of 179 previous commits were identified; of them 86 were the cause of the bug, while 82 were not. \grex{Pon ejemplos, a ser posible reales, de esto, porfa.}\gema{perfecto, ahora busco un ejemplo y lo anado abajo}
