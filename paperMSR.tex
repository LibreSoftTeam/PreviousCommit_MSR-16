...
With this paper we try to shed some ligth on how and when the bug was inserted into the source code, showing different empirical results from the the current assumption about the bug was originated in the previous modifications of the lines involved in the fix bug. Thus, our idea is based on the thought that some modifications or additions into the code source don't introduce any bug in the software. Our results could lead to think that all the articles based on this assumption are not using the best data in their results.On the other hand, this could be very useful in future research deleting this previous commits from the cause of a bug in their studies and could help developers understanding the bug seeding process. \grex{reescribir este parrafo para que quede mas claro.}
