...
This analysis takes into account that the bug could have been inserted in many lines that belong to many different previous commits, but in fact, not all of them could cause the bug. It may happen that in the previous commit, lines have been copied from further previous commits, comments may have been modified or blank spaces/lines have been introduced. Thus, the cause could be found in a single previous commit, in many or even in none. In the \ref{fig:7} we see that the previous commits are more than one, in this case two \textit{e7be0a988} and \textit{e5296c1da}. The previous commit \textit{e7be0a988} didn't cause the bug because it's a line to know the version of the file. The previous commit \textit{e5296c1da} caused the bug because it inserted a incorrect \textit{break} line. \grex{Todavia no esta del todo claro... yo creo que esto merece un ejemplo - incluso con codigo real, tal y como hemos hecho al principio del articulo. Ademas, nos lo podemos permitir; tenemos espacio.}\gema{perfecto, ahora busco un ejemplo y lo anado abajo}
...
As can be seen in Table~\ref{tab:responsability}, from the 308 previous commits, 152 have been considered to be the cause of the bug, whereas 114 were identified as not beign the cause. We were unable to decide in 42 cases, as only lines had been added \ref{fig:8} or due to our limited knowleadge about the code \ref{fig:9}. \grex{Gema, puedes poner un ejemplo de estas dos cosas? Uno de solo added lines y otro de limited knowledge. Si el ejemplo es verdadero, mejor.}\gema{perfecto, ahora busco un ejemplo y lo anado abajo}
...
\begin{figure}[ht]
\centering
\includegraphics[height=2cm]{addedlines.png}
